...
What to do now. Read after `INDEX.md`. Bullets ≤30 words, each naming a task ID or detail file.
`Recent sessions` holds the last five; older entries roll into `JOURNAL.md`.
...
- The backward-fill imputation rule in `T101`'s Notes was chosen by Claude and is not yet confirmed
  — confirm it before relying on it in the writeup.
...
- **2026-07-19** — Revised `D01` to backward-fill + validity flag; rejoined the new flags into
  `T103`'s dataset. See `ARCHIVE.md` (Decisions, format v2).
