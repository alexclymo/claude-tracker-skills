...
A self-contained, **non-working** illustration of the `tracker/` task system, applied
...
3. A task's detail file (`tracker/detail/T###_*.md`) — the full history behind whatever's in
   progress, including any ruling folded into its Notes.

Three things are worth noticing, because they are the format's whole reason to exist:

- **The two old rulings now live in task Notes.** The imputation rule is folded into `T101`'s
  Notes, and the ≥3-wave sample restriction into `T103`'s Notes — each states the current ruling
  and points to `ARCHIVE.md`, where the pre-migration text is preserved verbatim.
- **`T103`'s note carries an inline scope tag, `[spec: fixed-effects]`.** It binds only the
  fixed-effects robustness specification, not the pooled baseline. A fact that isn't universal is
  tagged so nobody over-applies it.
